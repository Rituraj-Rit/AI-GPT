      ...
      {messages.map((m,index) => (
        <div key={index} className={`msg msg-${m.type}`}>
          ...
          <div className="msg-actions" role="group" aria-label="Message actions">
            <button type="button" aria-label="Copy message" onClick={() => navigator.clipboard.writeText(m.contents)}>
            ...
            {m.role === 'ai' && (
              <>
                ...
                <button type="button" aria-label="Speak message" onClick={() => { try { const u = new SpeechSynthesisUtterance(m.contents); speechSynthesis.speak(u);} catch { /* speech synthesis unsupported */ } }}>
